...
public class IALowatem {
    ...
    void jouer(Case[][] plateau, int nbToursJeu) throws IOException {
        ...
        char couleurJoueur = plateau[Coordonnees.carLigneVersNum(actionsPossibles[0].charAt(0))]
             [Coordonnees.carColonneVersNum(actionsPossibles[0].charAt(1))].couleurUnite;
    ...
    void DifAttaque(int[][] attaques, String actionPossible, int indice, char couleurJ, NbPointsDeVie nbPvAvant){
        ...
        if (superieur(attaques, difPv, indice)){
            ajoutDif(attaques, difPv, indice);
        }
    }
    
    boolean superieur(int[][] attaques, int difPv, int indice){
        boolean place = false;
        for (int i = 0; i < attaques[0].length; i++){
            if (attaques[2][i] == 0 || difPv > attaques[0][i]){
                place = true;
            }
        }
        return place;
    ...
    /**
     * Calcul de la probabilité que le joueur adverse puisse attaquer le tour suivant
    ...
    int choixIndice(int[][] diffPv){
        int indiceChoisi = 0;
        int[][] indicesEnvisages = new int[2][20];
